...
public class Hand {
   ...
   public boolean isFlush() {
      ...
      for (Card c : hand) suits.add(c.getSuit());
      return suits.size() == 1;
   }
   ...
   public boolean isStraight() {
      ...
      for (Card c : hand) rankSet.add(c.getRank());
      return rankSet.size() == 5 && (cardRanks[0] - cardRanks[4] == 4);
   }

   // Is this hand three of a kind (also called a set or trips in poker)?
   public boolean isThreeOfaKind() {
      return getRankFrequency().containsValue(3) && getRankFrequency().size() == 3;
   }

   // Does this hand contain exactly two pairs?
   public boolean isTwoPair() {
      return getRankFrequency().containsValue(2) && getRankFrequency().size() == 3;
   }
